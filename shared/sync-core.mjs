...
export function stampLocal(meta, ops, clock) {
  ...
  for (const op of ops) {
    ...
    else if (op.t === 'add') meta.rec[keyOf(op.c, op.v.id)] = { a: stamp }
    else {
      const record = (meta.rec[keyOf(op.c, op.id)] ??= {})
      if (op.t === 'del') record.d = stamp
      else for (const field of [...Object.keys(op.v || {}), ...(op.unset || [])]) (record.f ??= {})[field] = stamp
    }
...
export function mergeEntries(doc, meta, entries) {
  ...
  for (const [stamp, op] of [...entries].sort((a, b) => compareStamps(a[0], b[0]))) {
    ...
    if (op.t === 'add') {
      // An add loses to a later delete, and to a later add of the same record.
      if (!newer(stamp, record.d) || !newer(stamp, record.a)) continue
      if (present(op.c, id)) {
        // Both devices made the same record (a day's page): merge it field by field.
        const v = {}
        for (const [field, value] of Object.entries(op.v)) {
          if (field !== 'id' && newer(stamp, fieldStamp(record, field))) {
            v[field] = value
      ...
      meta.rec[keyOf(op.c, id)] = { a: stamp, ...(Object.keys(keep).length ? { f: keep } : {}) }
    ...
    if (op.t === 'patch') {
      ...
      const unset = []
      for (const [field, value] of Object.entries(op.v || {})) {
        if (field !== 'id' && newer(stamp, fieldStamp(record, field))) { v[field] = value; (record.f ??= {})[field] = stamp }
      }
...
export function snapshotEntries(doc, meta) {
  ...
  walkDoc(doc, {
    record: (collection, item, at) => {
      ...
      if (!record?.a) return
      entries.push([record.a, { t: 'add', c: collection, v: item, at }])
      const later = Object.entries(record.f || {}).filter(([field, stamp]) => stamp > record.a && field in item)
      for (const [field, stamp] of later) entries.push([stamp, { t: 'patch', c: collection, id: item.id, v: { [field]: item[field] } }])
    },
    value: (path, current) => {
